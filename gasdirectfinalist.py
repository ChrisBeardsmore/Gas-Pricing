# Direct the Final 1 – Full Script (Stable Version)
# ---------------------------------------------------------------------------------
# ✅ Unified input grid (one row per site)
# ✅ LDZ postcode matching + flat file lookup
# ✅ Uplift entry in pence (SC ≤ 100, Unit ≤ 3.000)
# ✅ TAC + Dyce Margin calculation
# ✅ Customer output = Sell Standing Charge, Sell Unit Rate, TAC only
...
if uploaded_file:
    ...
    # Logo just above the input grid (top-right)
    try:
    ...
    st.subheader("🔧 Multi-site Input Grid")
    st.caption("Enter uplifts in **pence** – Standing Charge max = 100p/day, Unit Rate max = 3.000p/kWh")
    ...
    for i in range(10):
        ...
        with st.container():
            cols = st.columns([1.2, 1.2, 1] + [1]*3*3)  # 3 durations × 3 columns = 9 uplift blocks
    ...
    # ----------------------------
    if input_rows:
        customer_output = pd.DataFrame(input_rows)

        st.subheader("📊 Customer-Facing Output")
        st.dataframe(customer_output, use_container_width=True, height=400)

        st.subheader("📌 Summary")
        total_tac = customer_output[[col for col in customer_output.columns if "TAC" in col]].sum().sum()
        st.metric("Total TAC (All Sites)", f"£{total_tac:,.2f}")

        # ----------------------------
        # SECTION 5 – Export to Excel
        # ----------------------------
        st.subheader("📥 Download Customer Quote")

        output = io.BytesIO()
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            customer_output.to_excel(writer, index=False, sheet_name="Customer Quote")
        output.seek(0)
