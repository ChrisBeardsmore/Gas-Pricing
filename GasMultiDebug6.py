...
if uploaded_file:
    ...
    # --- Debug: Show actual column headers ---
    st.write("Loaded Columns from Supplier File:", df.columns.tolist())

    # --- Rename truncated columns if necessary ---
    df.rename(columns={
        "Minimum_A": "Minimum_Annual_Consumption",
        "Maximum_A": "Maximum_Annual_Consumption"
    }, inplace=True)

    # Clean columns
    df["LDZ"] = df["LDZ"].astype(str).str.strip().str.upper()
    df["Exit_Zone"] = df["Exit_Zone"].astype(str).str.strip().str.upper()
    df["Contract_Duration"] = pd.to_numeric(df["Contract_Duration"], errors='coerce').astype(int)
    ...
    contract_filter_months = st.selectbox("Contract Duration (months)", options=[12, 24, 36])
        ...
        if postcode and kwh > 0:
            ...
            if not match.empty:
                ...
                tariffs = df[
                    ...
                    (df["Contract_Duration"] == contract_filter_months)
                ]

                debug_info += f"Filters:\n - LDZ: {ldz}\n - Exit Zone: {exit_zone}\n - kWh: {kwh}\n - Contract: {contract_filter_months} months\n"
