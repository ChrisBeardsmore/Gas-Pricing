...
if uploaded_file:
    ...
    # Consumption for total annual cost
    ...
    # Convert Standing Charge to pence (multiply by 100)
    df_final["Standing Charge"] = (df_final["standingcharge"] + df_final["uplift_standing"]) * 100
    ...
    st.dataframe(df_final[["Unit Rate", "Standing Charge", "TotalAnnualCost"]].head())

    # Prepare Excel output
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        # Drop unused columns 17-29 if present
        all_cols = df_final.columns.tolist()
        clean_cols = [c for c in all_cols if not c.startswith("unnamed")]
        df_export = df_final[clean_cols]

        # Remove "unitrate" and "standingcharge", "uplift_unit", "uplift_standing"
        df_export = df_export.drop(
            columns=["unitrate", "standingcharge", "uplift_unit", "uplift_standing"],
            errors="ignore"
        )

        # Rename columns already renamed in df_final, so no extra renaming here
