...
def apply_uplifts(df, uplift_sc, uplift_unit):
    df['Standing Charge (p/day) Final'] = (df['Standing Charge (p/day)'] + uplift_sc).round(3)
    df['Unit Rate (p/kWh) Final'] = (df['Standard Rate (p/kWh)'] + uplift_unit).round(3)
    return df
...
if uploaded_file:
    ...
    st.subheader("Enter Uplifts Per MPAN")
    df['S/C Uplift (p/day)'] = 0.000
    df['Unit Rate Uplift (p/kWh)'] = 0.000
    uplift_editor = st.data_editor(df[['MPAN', 'S/C Uplift (p/day)', 'Unit Rate Uplift (p/kWh)']], num_rows='dynamic')

    if st.button("Apply Uplifts and Generate Broker Output"):
        df = df.merge(uplift_editor, on='MPAN', suffixes=('', '_input'))
        df['S/C Uplift (p/day)'] = df['S/C Uplift (p/day)_input']
        df['Unit Rate Uplift (p/kWh)'] = df['Unit Rate Uplift (p/kWh)_input']

        # Apply uplifts
        df = apply_uplifts(df, df['S/C Uplift (p/day)'], df['Unit Rate Uplift (p/kWh)'])

        # Pivot to single row per MPAN
        pivot_df = df.groupby(['Company Name', 'Company Reg', 'MPAN', 'Standard/Green', 'Contract Start Date', 'EAC (kWh)']).agg({
            else row.get('Night Rate (p/kWh)', 0), axis=1
        )
        df.drop(columns=['All Year - Night Rate (p/kWh)'], inplace=True)
    ...
    styled_df = df.style.applymap(
        lambda v: 'background-color: #D9EAF7' if isinstance(v, (int, float)) else '',
        subset=uplift_cols
    )
