...
sic_code = st.text_input("SIC Code (if known)")
...
def credit_decision_engine():
    ...
    # Optional SIC code lookup
    if sic_code:
        sic_df = load_sic_codes()
        matched = sic_df[sic_df['SIC_Code'] == sic_code]
        if not matched.empty:
            messages.append(f"SIC Lookup: Sector - {matched.iloc[0]['Sector']}, Typical Risk - {matched.iloc[0]['Typical_Risk_Rating']}")
        else:
            messages.append("SIC Code not found in reference data.")
