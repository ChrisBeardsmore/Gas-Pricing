...
if uploaded_file:
    ...
    header = ["Site", "Postcode", "kWh", "Cost Unit", "Cost SC", "Uplift Unit", "Uplift SC", "Final Unit", "Final SC", "Total Cost"]
    ...
    for i in range(10):
        ...
        site = cols[0].text_input("", key=f"site_{i}")
        postcode = cols[1].text_input("", key=f"postcode_{i}").replace(" ", "").upper()
        kwh = cols[2].number_input("", min_value=0, value=0, step=1000, key=f"kwh_{i}")
        ...
        uplift_unit = cols[5].number_input("", min_value=0.0, value=0.0, step=0.01, key=f"uplift_unit_{i}")
        uplift_sc = cols[6].number_input("", min_value=0.0, value=0.0, step=0.1, key=f"uplift_sc_{i}")
        ...
        cols[3].markdown(f"{cost_unit:.2f}")
        cols[4].markdown(f"{cost_sc:.2f}")
        cols[7].markdown(f"{final_unit:.2f}")
        cols[8].markdown(f"{final_sc:.2f}")
        cols[9].markdown(f"**£{total_cost:.2f}**")
