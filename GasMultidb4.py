...
if uploaded_file:
    ...
    contract_years = st.selectbox("Contract Duration (years)", [1, 2, 3])
    ...
    for i in range(10):
        ...
        cols = st.columns([1, 1.5, 1.5, 1, 1, 1, 1, 1, 1, 1.5, 1.5])
        ...
        if postcode and kwh > 0:
            ...
            if not match.empty:
                ...
                # FIX: Get the tariff with the lowest Unit_Rate if multiple rows match
                tariffs = df[
                    (df["LDZ"] == ldz) &
        ...
        cols[10].metric("Total Annual Cost (£)", f"{total_cost:.2f}")
