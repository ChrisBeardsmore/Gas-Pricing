...
def apply_uplifts_per_row(df, rate_columns):
    ...
    for col in rate_columns:
        if col not in excluded_cols:
            ...
            df[final_col] = (df[col].fillna(0) + df.get(uplift_col, 0)).round(3)
    return df
...
if uploaded_file:
    ...
    if 'All Year - Day Rate (p/kWh)' in df.columns:
        df['Day Rate (p/kWh)'] = df.apply(
            lambda row: row['All Year - Day Rate (p/kWh)'] if not pd.isna(row['All Year - Day Rate (p/kWh)'])
            else row.get('Day Rate (p/kWh)', 0), axis=1
        )
        df.drop(columns=['All Year - Day Rate (p/kWh)'], inplace=True)

    if 'All Year - Night Rate (p/kWh)' in df.columns:
        df['Night Rate (p/kWh)'] = df.apply(
            lambda row: row['All Year - Night Rate (p/kWh)'] if not pd.isna(row['All Year - Night Rate (p/kWh)'])
