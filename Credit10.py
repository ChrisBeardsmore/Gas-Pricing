...
def credit_decision_engine():
    def score_creditsafe(cs):
        return 100 if cs >= 80 else 75 if cs >= 60 else 50 if cs >= 40 else 25

    def score_years_trading(yt):
        return 100 if yt > 5 else 75 if yt >= 2 else 50 if yt >= 1 else 25
    ...
    def score_contract_value(val):
        return 100 if val < 25000 else 75 if val <= 50000 else 50 if val <= 100000 else 25
    ...
    if decision in ["⚠️ Approved with Stipulations", "🔍 Refer / Manual Review"]:
...
def export_to_excel(result):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        df.to_excel(writer, index=False, sheet_name='Credit Decision')
    output.seek(0)
    return output
...
if st.button("Run Credit Decision"):
    with st.spinner("Calculating..."):
        ...
        st.download_button(
            label="📥 Download Credit Decision Report",
            data=excel_data,
            file_name=f"Credit_Decision_{business_name}.xlsx",
